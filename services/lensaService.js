...
module.exports.getCandidate = async (url, item) => {
  let fileName
  let response
  ...
  await this.saveCandidateInfo(item, response.data, fileName)
}

module.exports.saveCandidateInfo = async (data, value = null, fileName = null) => {
  let externalId = data.email
  const source = data.candidate_source
  let rawCandidateModel = await rawCandidateRepository.create({ source_type: source || 'lensa', external_id: externalId, data }).catch((reason) => {
    console.log('rawCANDIDATEINSERT', reason)
    counterError++
