...
feedProcess(process.argv[2], process.argv[3], process.argv[4] || 14, process.argv[5] || 3, process.argv[6] || 200, process.argv[7] || false).catch(
  (err) => {
    console.log(err)
  },
)
    ...
    https
      .get(urlParam, async (response) => {
        if (response && response.statusCode >= 400) {
          ...
          await feedProcessingRepository.update({ status: 'error' }, feedProcessingId).catch((err) => console.log(err.message))
          process.exit(1)
        }
        response.pipe(file)
        file.on('finish', () => {
  ...
  let result = await new Promise((resolve, reject) => {
    ...
    xml.on('endElement: item', async function (item) {
      ...
      if (matchCandidatesPromises.length === parseInt(parallelProcess)) {
        xml.pause()
        try {
          await Promise.all(matchCandidatesPromises)
        } catch (err) {
          await dbLogger(err, 'Matcherror', {
            urlParam: urlParam,
            feedProcessingId: feedProcessingId,
            lastUpdateDays: lastUpdateDays,
            candidateQuantityPerJob: candidateQuantityPerJob,
      ...
      try {
        await feedProcessingRepository.increment('jobs', 1, { id: feedProcessingId })
      } catch (err) {
        await dbLogger(err, 'Matcherror', {
          urlParam: urlParam,
          feedProcessingId: feedProcessingId,
    ...
    xml.on('endElement: channel', async (channel) => {
      channel.item = null
      await feedProcessingRepository.update({ details: channel }, feedProcessingId).catch((err) => {
        console.log(err)
        logger.info(JSON.stringify({ processingId: feedProcessingId, err }))
      })
    })
    xml.on('error', async (err) => {
      await dbLogger(err, 'xmlParseError', {
        urlParam: urlParam,
        feedProcessingId: feedProcessingId,
        lastUpdateDays: lastUpdateDays,
        candidateQuantityPerJob: candidateQuantityPerJob,
      ...
      console.log('XMLERRROR', err)
      await feedProcessingRepository.update({ status: 'interuppted' }, feedProcessingId).catch((err) => console.log(err.message))
      process.exit(1)
    })
    xml.on('end', async (value) => {
      ...
      if (matchCandidatesPromises.length > 0) {
        await Promise.all(matchCandidatesPromises).catch((err) => console.log(err))
      }
      ...
      console.log('endit', matchCandidatesPromises.length)
      process.exit(0)
    })
...
async function matchCandidates(searchRequest, item, feedProcessingId, lastUpdateDays, candidateQuantityPerJob, harversterMode) {
  ...
  let searchResponse = await candidateControllerService.getCandidates(searchRequest, 1, 1, true).catch((err) => {
    console.log('getcandidates EXCEPTION', err)
  })
  // get semantic search
  if (!_.isUndefined(searchResponse)) {
        ...
        if (!_.isEmpty(candidates)) {
          ...
          for (const candidate of candidates) {
            ...
            if (!_.isUndefined(dbCandidate) && !_.isNull(dbCandidate)) {
              let dbCandidateResumeModifiedDate = dbCandidate.data.identity.resumeModifiedDate
              ...
              if (Date.parse(dbCandidateResumeModifiedDate) + 7 * 60 * 60 * 24 * 1000 >= Date.parse(candidateResumeModifiedDate)) {
                ...
                promises.push(
                  candidateControllerService
                    .getCandidate(textResumeId, firstBoard, true, jobId, feedProcessingId, jobFoundCount)
                    .then(async (data) => {
                      await feedProcessingRepository.increment('candidates', 1, { id: feedProcessingId }).catch(async (error) => {
                        console.log(error)
              ...
              promises.push(
                candidateControllerService.getCandidate(textResumeId, firstBoard, true, jobId, feedProcessingId, jobFoundCount).then(async (data) => {
                  await feedProcessingRepository.increment('candidates', 1, { id: feedProcessingId }).catch(async (err) => {
                    console.log(err)
                    await dbLogger(err, 'Matcherror', {
                      urlParam: urlParam,
                      feedProcessingId: feedProcessingId,
